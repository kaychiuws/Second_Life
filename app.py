...
if not st.session_state.game_started:
    ...
    def run_turn(choice_text=""):
        ...
        3. 如果本回合沒有因【上一步抉擇】失去任何資產，請務必回傳空陣列 `[]`。

        【資產主動性與防呆鐵律】：
        1. 若玩家背包中有特殊資產，請盡量在3個選項中設計一個「使用該資產破局」的選項。
        2. 若因抉擇失去資產，填寫 `lost_assets` 時必須寫出「完整資產名稱」，絕不可只寫單字（例如不可只寫「糧」，必須寫「糧票」）。
        
        【敘事節奏與NPC鐵律】：
        1. 不要每回合都是抄家毆打的極端暴力。有時危機應是「平庸的惡」（例如分配口糧時的心理博弈），用冷暴力和人性的扭曲來施壓。
        2. 請將玩家的 `npc_updates`（壓迫者、依附者、盟友）融入你的故事中，讓玩家的背叛或互助有具體的對象。
