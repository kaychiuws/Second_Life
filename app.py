...
if not st.session_state.game_started:
    ...
    def run_turn(choice_text=""):
        ...
        if p_state["game_stage"] == "prologue":
            ...
            這是遊戲的開局（1957年）。請為玩家設計一個充滿希望與鼓勵言論自由氛圍的場景。
            你的文本必須溫情脈脈，3 個物理選項必須刻意迎合「現代人的常識與普世價值」（如：提出建設性批評、反映基層困難）。
            絕對不要暗示危機，讓現代玩家毫無防備地踩入陷阱。
            """
            paragraph_one_rule = "[段落一（日常底色）]：作為開局，請用1-2句文學語言，描寫玩家當下基於其【背景與資產】的日常生活狀態，作為時代風暴前的寧靜。"
            
        else:
            # 1958年及以後：常規過渡
            paragraph_one_rule = "[段落一（蒙太奇餘波）]：用1-2句充滿感官細節的文學語言，描寫「上一步抉擇」在隨後幾個月裡帶來的餘波、磨耗或短暫平靜，展現時間的流逝。"
            ...
            if p_state["current_year"] == 1958 and choice_text:
                ...
                請在第一段中，用最殘酷的史實擊碎玩家的現代思維：他們上一步的建議，現在被定性為「右派言論」。
                請在 `stat_changes` 中給予沉重打擊（健康或理智 -20，共業飆升），並剝奪相關資產。隨後切入1958年的新危機。
                """
            elif 1962 <= p_state["current_year"] <= 1965:
                # 🌟 1962-1965：歷史的喘息（大幅回血期）
                special_directive = """
                【歷史喘息鐵律：調整期的溫情（極機密）】：
                歷史進入1962-1965年的「調整、鞏固、充實、提高」時期。大饑荒已過，文革尚未爆發，政策放寬，民間迎來久違的生機。
                請在故事中展現食物配給的增加、黑市的活躍或生活秩序的恢復。
                你的3個選項必須偏向「生活重建」與「休養生息」，並在 `stat_changes` 中給予慷慨的正向回報（例如：健康 +10 到 +20，理智 +10 到 +20），讓玩家在這四年間能大幅「回血」，儲備未來文革的生存本錢。
                """
            else:
                # 🌟 常規年份：平凡的日常（微量回血）
                special_directive = """
                【日常罅隙鐵律：風暴邊緣的平靜（極機密）】：
                即使在嚴酷的年份，普通人依然要吃飯、睡覺，在風暴邊緣尋找苟活的空間。
                請在你的3個選項中，務必保留至少一個「平凡的日常抉擇」（例如：專心勞動換取口糧、與家人分享僅有的食物、在角落偷看舊書、鄰里間的微小互助）。這種選項應帶來微小但真實的回血（健康 +5 或 理智 +5），不增加共業，讓玩家得以喘息。
                """

        # 🌟 精簡並重構後的系統指令
        system_instruction = f"""
        角色：擁有豐富經驗的頂級小說家，文字風格細膩、極具畫面感與沉浸感。
        禁忌詞：{json.dumps(taboo, ensure_ascii=False)}（絕對禁止使用宏觀定性名詞，需用客觀感官拆解描寫）
        
        【敘事與排版結構】
        請嚴格輸出三段無標題純文字，段落間以 `\\n\\n` 分隔：
        {paragraph_one_rule}
        2. 時代氣候（最重要）：為了讓文章立體且生動，你的生成內容必須自然且交錯地包含以下五個元素。請不要生硬地分段或像列點一樣陳述，而是將它們行雲流水地編織在敘事之中：
           環境氛圍：描寫當下的光影、天氣或場景基調（如壓抑、輕快、詭異等）。
           五感描寫：至少包含兩種以上的感官體驗（視覺、聽覺、嗅覺、觸覺、味覺），讓場景具體化。
           群眾神態：描寫周遭旁人、路人或背景人物的反應、微表情或動作，以此烘托主角的處境。
           個人內在想法：深刻刻畫主角（或視角人物）當下的心理活動、情緒波動或回憶。
           對話元素：包含主角與他人的對話，或是主角無聲的喃喃自語。
           注意事項：
           保持文字流暢，視角要一使用第一人稱「我」
           描寫要克制但精準，避免過度堆砌華麗但無意義的辭藻。
        3. 命運迫近：將時代危機具象化為眼前逼迫玩家的一件日常衝突，引出接下來的 3 個物理抉擇。
        
        【選項與數值設計】
        1. 數值克制：單次變動限制在 -15 到 +20 之間。
        2. 生存的張力：除了出賣良知換取苟活的「黑暗三角折衷」外，必須依據【動態鐵律】提供能讓玩家吃飯、休息或互助的「回血選項」，讓生命得以延續。
